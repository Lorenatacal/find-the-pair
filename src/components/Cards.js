...
function Cards() {
    const [cards, setCards] = React.useState(shuffle(generateCards()))
    ...
    function matchCards({}, index) {
      ...
      handleCardHistoryChanges(copyOfCards, index)
    }

    const handleCardHistoryChanges = (copyOfCards, index) => {
      if(cardHistory.length < 2 ) {
        ...
        handleCheckForPair(updatedCardHistory)    
      }
    }

    const handleCheckForPair = (updatedCardHistory) => {
      if (updatedCardHistory.length !== 2) return

      const isAMatchingPair = updatedCardHistory[0].type === updatedCardHistory[1].type
      if (isAMatchingPair) {
        setScore(score + 1)
        setCardHistory([])
